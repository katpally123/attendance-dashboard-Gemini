...
function toISODate(d){
    ...
    if ((m=mdy.exec(noTime))){
        const [,mm,dd,yyyy]=m; return new Date(`${yyyy}-${mm.padStart(2,"0")}-${dd.padStart(2,"0")}T12:00:00`).toISOString().slice(0,10);
    }
    if ((m=ymd.exec(noTime))){
        const [,yyyy,mm,dd]=m; return new Date(`${yyyy}-${mm.padStart(2,"0")}-${dd.padStart(2,"0")}T12:00:00`).toISOString().slice(0,10);
    }
    return null;
}
    ...
    try{
        ...
        if (vetRaw && vetRaw.length) {
            ...
            for (const r of vetRaw) {
                ...
                // 7. Deduping on Employee ID, Date, Type, and Opportunity ID
